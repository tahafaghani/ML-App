def runscript():
    ...
    from sklearn.metrics import accuracy_score, recall_score, precision_score, confusion_matrix, classification_report, ConfusionMatrixDisplay, f1_score
    ...
    # Optionally replace cells with only spaces with NaN
    # df = df.replace(r'^\s*$', pd.NA, regex=True, inplace=True)
    ...
    train_acc = model_knn.score(X_train, y_train)
