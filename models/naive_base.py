def runscript():
    ...
    y_pred = model_naive_base.predict(X_test)
    ...
    train_acc = model_naive_base.score(X_train, y_train)
    test_acc = model_naive_base.score(X_test, y_test)
    print(f" Training Accuracy: {train_acc:.4f} \n Test Accuracy: {test_acc:.2}")

    # Display the confusion matrix
    cm = confusion_matrix(y_test, y_pred, labels=model_naive_base.classes_)
    disp = ConfusionMatrixDisplay(confusion_matrix=cm, display_labels=model_naive_base.classes_)
    disp.plot()
    plt.show()
